...
fit_homoskedastic_models <- function(model_data) {
  settings <- stage2_mcmc_settings()
  rows <- list()
  prior_rows <- list()

  for (p in c(1L, 2L)) {
    model_id <- paste0("BVAR-HOM-p", p, "-pilot")
    file <- file.path("output", "models", paste0("bvar_hom_p", p, "_pilot.rds"))
    log_msg("Estimating", model_id, "burnin=", settings$pilot_burnin, "draws=", settings$pilot_draws)
    start <- Sys.time()
    fit_bvar_model(
    ...
    rows[[length(rows) + 1]] <- data.frame(
      model_id = model_id, lag = p, variance_type = "homoskedastic",
      run_type = "pilot", chain = NA_integer_, seed = 20260 + p,
      burnin = settings$pilot_burnin, draws = settings$pilot_draws,
      thin = settings$thin, elapsed_seconds = elapsed, file = file,
      stringsAsFactors = FALSE
    )
    prior_rows[[length(prior_rows) + 1]] <- prior_spec_row(model_id, p, "homoskedastic", FALSE)
  }

  p_selected <- read_candidate_lag()
  for (chain in seq_along(c(20261L, 20262L, 20263L))) {
    seed <- c(20261L, 20262L, 20263L)[chain]
    model_id <- paste0("BVAR-HOM-final-chain", chain)
    file <- file.path("output", "models", paste0("bvar_hom_final_chain", chain, ".rds"))
    log_msg("Estimating", model_id, "p=", p_selected, "burnin=", settings$final_burnin, "draws=", settings$final_draws)
    start <- Sys.time()
    fit_bvar_model(
      model_data = model_data,
      p = p_selected,
    ...
    elapsed <- as.numeric(difftime(Sys.time(), start, units = "secs"))
    rows[[length(rows) + 1]] <- data.frame(
      model_id = model_id, lag = p_selected, variance_type = "homoskedastic",
      run_type = "final", chain = chain, seed = seed,
      burnin = settings$final_burnin, draws = settings$final_draws,
      thin = settings$thin, elapsed_seconds = elapsed, file = file,
      stringsAsFactors = FALSE
    )
    prior_rows[[length(prior_rows) + 1]] <- prior_spec_row(model_id, p_selected, "homoskedastic", FALSE)
  }
  ...
  write_csv_safe(model_rows, file.path("output", "tables", "bvar", "bvar_homoskedastic_estimation_runs.csv"))
...
main <- function() {
  ...
  log_msg("BVAR homoskedastic estimation started.")
  data <- load_stage2_data()
  fit_homoskedastic_models(data$model_data)
  log_msg("BVAR homoskedastic estimation completed.")
}
